...
namespace IndustrialCommSdk
{
    ...
    public sealed class IndustrialDeviceHost : IDisposable
    {
        ...
        /// <summary>从 devices.json 加载所有已启用设备。</summary>
        public static IndustrialDeviceHost Load(string configFilePath, IIndustrialLogger logger = null)
        {
            ...
            return new IndustrialDeviceHost(
                config,
                Path.GetDirectoryName(fullConfigPath),
                device => IndustrialClientFactory.FromConfig(device, logger),
                logger);
        }
            ...
            IIndustrialLogger logger = null)
        {
            ...
            {
                if (deviceConfig == null || !deviceConfig.Enabled.GetValueOrDefault(true))
                {
                    continue;
                }
                ...
                if (deviceConfig.PollingIntervalMilliseconds.HasValue && deviceConfig.PollingIntervalMilliseconds.Value <= 0)
                {
                    throw new InvalidOperationException("pollingIntervalMilliseconds must be greater than zero for device: " + deviceConfig.Name);
                }

                if (deviceConfig.ReconnectDelayMilliseconds.HasValue && deviceConfig.ReconnectDelayMilliseconds.Value <= 0)
                {
    ...
    public sealed class IndustrialHostedDevice : IDisposable
    {
        ...
        {
            var delay = TimeSpan.FromMilliseconds(_config.ReconnectDelayMilliseconds.GetValueOrDefault(3000));
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (!Device.Client.IsConnected ||
                        ...
                        (_config.PollingIntervalMilliseconds.HasValue && string.IsNullOrWhiteSpace(_subscriptionId)))
                    {
        ...
        private async Task ConnectAndSubscribeAsync(bool reconnecting, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (_config.PollingIntervalMilliseconds.HasValue && string.IsNullOrWhiteSpace(_subscriptionId))
                {
                    var request = new SubscriptionRequest(
                        Device.DeviceName + ".devicehost",
                        Device.Client.DeviceId,
                        CreateReadRequests(),
                        TimeSpan.FromMilliseconds(_config.PollingIntervalMilliseconds.Value),
                        _config.ReportOnChangeOnly.GetValueOrDefault(false));
