...
namespace InduLinkDemo.Views
{
    ...
    public partial class Snap7ServerTab : UserControl
    {
        ...
        {
            const string fileName = "InduLinkDemo.Snap7Server.exe";
            var baseDirectory = AppDomain.CurrentDomain.BaseDirectory;
            var candidates = new[]
            {
                Path.Combine(baseDirectory, "snap7server", fileName),
                Path.Combine(baseDirectory, fileName),
                Path.GetFullPath(Path.Combine(baseDirectory, "..", "..", "..", "InduLinkDemo.Snap7Server", "bin", "Debug", "net8.0", fileName)),
                Path.GetFullPath(Path.Combine(baseDirectory, "..", "..", "..", "InduLinkDemo.Snap7Server", "bin", "Release", "net8.0", fileName)),
                Path.GetFullPath(Path.Combine(baseDirectory, "..", "..", "..", "InduLinkDemo.Snap7Server", "bin", "x86", "Debug", "net8.0", fileName)),
                Path.GetFullPath(Path.Combine(baseDirectory, "..", "..", "..", "InduLinkDemo.Snap7Server", "bin", "x86", "Release", "net8.0", fileName)),
            };
            var executable = candidates.FirstOrDefault(File.Exists);
            if (executable == null)
            {
                throw new FileNotFoundException(
                    "找不到 InduLinkDemo.Snap7Server.exe。请先生成 InduLinkDemo.Snap7Server 项目。",
                    fileName);
            }
            return executable;
        }
