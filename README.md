...
协议模块互不引用，也不引用聚合程序集。第三方驱动包只存在它的所属模块中。

Redis 保持为独立的 `IndustrialCommSdk.Protocols.Redis`，用于键值访问和缓存集成；它不是 SQL Server/MySQL 历史库的方言实现，不进入关系型数据库选择器。
...
using IndustrialCommSdk.Storage.MySql;         // MySQL 历史存储提供程序
```
...
SQL Server 和 MySQL 实现同一个 `IIndustrialHistoryStore`，采集与查询代码不需要分支：

```csharp
using (IIndustrialHistoryStore store = useMySql
    ? (IIndustrialHistoryStore)new MySqlIndustrialDataStore(new MySqlDataStoreOptions
      {
          ConnectionString = mySqlConnectionString,
          TableName = "IndustrialDataHistory"
      })
    : new SqlServerIndustrialDataStore(new SqlServerDataStoreOptions
      {
...
MySQL 支持 `table` 或 `database.table`，服务端要求 MySQL 8.0+；它用 UTC 时间和偏移分钟两列保留 `DateTimeOffset` 语义。完整配置、缓冲写入和查询示例见 [历史存储索引](docs/storage/README.md)、[SQL Server](docs/storage/sql-server.md) 与 [MySQL](docs/storage/mysql.md)。

只引用 `IndustrialCommSdk.Storage` 时不会携带 MySQL 驱动；完整聚合入口 `IndustrialCommSdk` 按定义包含全部内置模块，因此会同时带上 MySQL 提供程序。
...
`MesJsonReceiver` 可通过本地 `HttpListener` 接收任意相对路径的 POST JSON 对象，支持正文上限、处理超时、Authorization 完整值检查和自定义 JSON 响应。`MaxConcurrentRequests` 默认 32；超过上限时快速返回 JSON `429`，已超时但未真正退出的处理器继续占用容量，避免慢请求或不响应取消的业务处理器造成无界任务累积。

## 运行时稳定性

- `PollingScheduler` 使用可取消的定时等待；最后一个订阅移除后终止对应 worker，Dispose 会等待活动与退役 worker，并隔离回调异常和回调内 Dispose。
- 客户端操作超时或外部取消后，尚未退出的协议核心任务仍独占连接，直到真正结束，防止下一请求并发复用同一 PLC 连接。
- `IndustrialDeviceHost` 的启动可回滚，失败停止可重试；构造中途失败会释放已经创建的客户端。
- 缓冲记录器串行化 Start/Stop/Dispose，停止后不允许重启；生产者与停止并发时只返回拒绝，不抛出队列竞态异常。
- TCP 使用连接代际隔离旧连接错误；断线、重连和分帧失败会清理对应残帧，排队接收的取消不会破坏活动接收的半帧。
- MES Receiver 的 Start/Stop/Dispose、处理超时、处理器内停止和异常 accept-loop 都有确定的状态收尾。
...
- 数据库页可切换 SQL Server / MySQL，两种提供程序各自保留连接与表名草稿；Redis 不在此页混用。
...
- 自定义协议核心任务如果完全忽略取消且协议级超时清理也无法中断 I/O，后续操作与 Dispose 会等待该任务退出；这是避免同连接并发访问的安全边界。
