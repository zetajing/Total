...
- 实时展示设备连接状态、最近错误和点位值；连接、轮询与重连全部由 SDK 的 `IndustrialDeviceHost` 执行。
- 运行中心采集值可直接进入 SDK 数据库记录器，协议页面继续作为现场调试工具保留。
- 一级导航只保留运行中心、设备配置和历史数据；协议直连、Socket、MES、网卡与存储集中在"调试与维护"。
- 设备配置提供常用参数表单和点位表格，JSON 作为高级编辑入口保留；保存和序列化由 SDK 配置模型负责。
...
`IndustrialCommSdk.Mes.MesTcpClient`，兼容《设备与 MES 通讯文档（FA）》及 H101_ST0 参考程序的无分隔符长连接协议：
...
`IndustrialCommSdk.Mes.MesHttpClient`，通过 REST API 与 MES 服务通信，无状态、无服务端推送：

1. 设备通过 HTTP POST 请求发送上线、FACHECK 和 FATRACK。
2. 使用 `DataContractJsonSerializer` 进行 JSON 序列化（与 TCP 模式一致）。
3. 内置超时控制和失败重试（仅对 5xx / 超时 / 网络异常重试）。
4. 无长连接，无需断线重连逻辑。

```csharp
using IndustrialCommSdk.Mes;

var mes = new MesHttpClient(new MesHttpClientOptions
{
...
// 发送上线（验证连通性）
var onlineResult = await mes.SendOnlineAsync(CancellationToken.None);

// 发送 FATRACK
var trackResult = await mes.SendFaTrackAsync(new MesFaTrackRequest
{
    Process = "FAFAL0290M0",
    SerialNo = "SN001",
    Number = "4",
...
// 发送 FACHECK
var checkResult = await mes.SendFaCheckAsync(new MesFaCheckRequest
{
    Process = "FAFAL0290M0",
    SerialNo = "SN001",
    Result = "OK",
...
Demo 的"MES"页新增"通讯模式"下拉框，可在 **TCP 长连接** 与 **HTTP API** 之间切换：

- **TCP 模式**：保持原有全部功能（连接/断开/自动重连/事件推送）。
- **HTTP 模式**：填写 API 地址替代主机+端口，发送上线验证连通性，FATRACK/FACHECK 直接发出并展示 HTTP 响应结果。
