...
namespace InduLink.Runtime
{
    ...
    public static partial class IndustrialClientQuickExtensions
    {
        ...
        private static T ConvertValue<T>(object value, string address, DataType dataType)
        {
            ...
            if (typeof(T) == typeof(DateTime) && value is DateTimeOffset dateTimeOffset)
            {
        ...
        private static DataType InferDataType(Type type)
        {
            ...
            if (targetType == typeof(sbyte)) return DataType.SByte;
            ...
            if (targetType == typeof(long)) return DataType.Int64;
            if (targetType == typeof(ulong)) return DataType.UInt64;
            if (targetType == typeof(float)) return DataType.Float;
            if (targetType == typeof(double)) return DataType.Double;
            if (targetType == typeof(TimeSpan)) return DataType.Time;
            if (targetType == typeof(DateTime) || targetType == typeof(DateTimeOffset)) return DataType.Date;
