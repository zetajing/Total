...
| `IndustrialCommSdk.Storage` | 历史存储契约、SQL Server、缓冲记录器、CSV |
| `IndustrialCommSdk.Storage.MySql` | MySQL 8.0+ 历史存储与 MySqlConnector |
| `IndustrialCommSdk` | 引用全部模块的聚合入口和默认注册表 |

原 `IndustrialCommSdk.Core` 已迁空并从解决方案删除。协议项目不互相引用，也不引用聚合入口；第三方包只属于它的所属模块。
...
## 2026-07-15 SQL Server / MySQL 历史存储

- `IndustrialCommSdk.Storage` 保留现有 SQL Server API 和通用模型，新增 `IIndustrialHistoryStore` 作为完整历史查询契约。
- `IndustrialCommSdk.Storage.MySql` 是独立提供程序，只该程序集直接引用 MySqlConnector 2.6.1。只引用 `IndustrialCommSdk.Storage` 的 SQL Server 用户不需携带 MySQL 驱动；引用完整聚合入口 `IndustrialCommSdk` 时会包含所有内置模块。
- `SqlServerIndustrialDataStore` 和 `MySqlIndustrialDataStore` 实现同一套初始化、批写、条件查询、分页、汇总、最新值、增量读取和删除能力。
- MySQL 服务端基线为 8.0+，表名可为 `table` 或 `database.table`；时间用 `TimestampUtc DATETIME(6)` 和偏移分钟两列保留偏移，并以微秒精度恢复 `DateTimeOffset`。
- SQL Server 历史查询现在与 MySQL 一样返回 `RawData`，便于协议字节故障排查。
- 分页总数与页数据在同一一致读取中产生；MySQL 汇总和时间边界由同一条语句返回。最新值查询先排名 Id 再回表读取 `RawData`，避免大字段参与窗口排序。
- WPF Demo 数据库页可切换 SQL Server / MySQL，两种草稿独立保存，并会把旧 SQL Server UI 状态自动迁移到新字段。启停使用单操作门控，并在连接前捕获不可变提供程序/连接/表名快照，避免重复启动和跨库读写。
- Redis 仍是独立 `IndustrialCommSdk.Protocols.Redis`，它是键值/缓存协议集成，不实现关系型历史库契约，不进入 SQL Server/MySQL 选择器。
- `docs/storage/` 提供总览与两种数据库的 `net472` 最小示例，并已加入解决方案文档树。
...
- `MesJsonReceiver` 接收任意相对路径的 POST JSON 对象，支持正文上限、处理超时、Authorization 检查和自定义 JSON 响应；`MaxConcurrentRequests` 默认 32，过载快速返回 JSON 429。
- WPF 与 WinForms MES 页面均只使用 JSON 配置和 JSON 正文。

## 2026-07-16 运行时稳定性修复

- `PollingScheduler` 不再用 `Task.WhenAny` 遗留输掉竞争的信号量 waiter；空 worker 会停止并从活动集合移除，被替换但尚未退出的 worker 会保留到 Dispose 完成。
- 回调异常继续隔离；回调直接 Dispose，以及回调完成普通 `TaskCompletionSource` 后内联执行的 Dispose，都不会自等待死锁。订阅模型在进入 worker 前拒绝 null 项。
- `IndustrialClientBase` 在超时或外部取消后继续让未退出的 core task 持有操作锁，并观察迟到异常，避免后续请求与旧请求并发访问同一连接。
- `IndustrialDeviceHost` 启动失败会回滚已启动设备；停止失败保留可重试状态和订阅 ID；构造中途失败会释放已经创建的客户端。
- `BufferedIndustrialDataRecorder` 串行化 Start/Stop/Dispose；初始化失败或取消不会发布 worker；停止后的实例不可重启；停止与生产者并发不会抛出 `BlockingCollection` 竞态异常。
- TCP 客户端使用不可变连接快照和连接代际，旧连接错误不会关闭新连接；显式断线、跨代重连、接收错误和分帧错误会清理残帧，排队接收取消不会清除活动接收的半帧。
- MES Receiver 在锁外有界等待在途请求，处理器内 Stop 不自等待；同步阻塞处理器也能返回 504。超时处理器继续受跟踪并占用容量，过载请求快速返回 429；Start/Dispose 与 accept-loop 异常退出不会留下伪 Running 状态。
- WPF MES 接收配置已加入 `maxConcurrentRequests`，旧 UI 状态缺少该字段时使用默认值 32。

## 测试状态

当前 Release 测试共 125 项，全部通过。除模块化、协议和存储测试外，稳定性覆盖新增：

- 七种协议 Settings 序列化往返与默认注册完整性。
- 未知协议、旧别名、重复注册、错误 Settings 类型和必填字段。
- 协议程序集不引用聚合入口或其他协议程序集。
- PollingScheduler 的定时唤醒、worker 替换、回调异常/内联延续隔离、取消和 Dispose。
- DeviceHost 启动回滚、停止重试、构造失败清理、禁用设备和无效点位文件。
- Storage 缓冲写入重试、计数、启动/停止竞态、初始化取消、生产者并发和优雅停止。
- TCP 断线/重连代际、跨连接残帧、排队取消、Dispose 与并发接收。
- MES Receiver 并发上限、429、504、处理器内停止、Start/Dispose 竞态和 accept-loop 异常退出。
- SQL Server/MySQL 共同历史契约、MySQL Options、安全表名、本地参数校验和预取消。
- `MySqlConnector` 只由 MySQL 存储提供程序引用，MySQL 导出类型不漂移出程序集命名空间。
- Redis 程序集不引用任何关系型 Storage 程序集，`RedisClient` 不实现历史存储契约。

最新验收：Release 全解决方案构建成功，0 错误；只有 OPC UA 依赖现有 API 的 6 个 obsolete 警告。Release 测试 125/125 通过，30 秒防挂保护未触发。此前七页协议示例编译、WPF 启动冒烟和 MySQL 依赖扫描结论保持有效。
...
- SQL Server 与 MySQL 真实服务器集成测试尚未纳入默认 CI；日常测试为不连外部数据库的离线合约与校验测试。
- 自定义协议 core task 如果完全忽略取消且 `OnOperationTimeout` 无法中断底层 I/O，后续请求和同步 Dispose 会等待该任务真正退出；该取舍用于保证同一连接不被并发访问。
...
5. 为 SQL Server 和 MySQL 8 增加可选容器集成测试，覆盖建表、事务回滚、时区往返和取消。
