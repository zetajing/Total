...
- 默认分支：`master`
- 当前远端提交：`668ad7b`（MES 重构为开放式 HTTP JSON）。
- P0/P1 合并提交：`1afa2eb44394361548f8fd3d313b7c939bed89ca`
- 核心扩展平台已通过 PR #2 合并到 `master`，合并提交为 `efd7b47`。
- “Demo 产品化与结构收敛”及“开放式 MES HTTP JSON”均已提交并推送到 `master`；当前工作区干净。
...
1. SDK 只保留开放式 HTTP JSON 客户端
   ...
   - 4xx 原样返回；5xx、网络异常和超时执行有界重试；保留响应大小限制、调用方取消和外部 `HttpClient` 生命周期语义。
...
4. 验证结果
   - `dotnet test IndustrialCommSdk.Tests/IndustrialCommSdk.Tests.csproj -c Release --no-restore`：47/47 通过。
...
- [x] 已恢复轻量 `IndustrialCommSdk.Tests` 回归工程；当前覆盖超时与诊断、TCP 分帧、配置、批量计划以及开放式 MES HTTP JSON，共 47 项测试。
...
- 当时新增 4 项 MES HTTP 自动化测试，验证 5xx 有界重试、重试恢复、4xx 不重试、处理器所有权及外部客户端生命周期；该历史阶段测试为 20/20 通过。当前基线已更新为 47/47，见“2026-07-13 MES HTTP JSON 收敛”。
